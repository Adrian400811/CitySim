 ...
 *
 * <ul>
 *   <li>Height is set automatically to fit. See below for a detailed description of how it's
 *       calculated. The easiest way to get the height in order to position it is to have it build
 *       itself, and then call getHeight() on the created object.
 *   <li>Border Thickness is optional --> Set it to zero (0) to create a box with no border
 *   <li>If you turn on centered, the processing cost goes up dramatically upon updates. This is
 *       really only an issue if you plan to publish to the Greenfoot Gallery. If you are, please
 *       consider not using centered, or updating very sparingly.
 * </ul>
 *
 * <p>To understand <b>height</b> examine the following equation: <code>
...
public class SuperTextBox extends Actor {
   ...
   * @param centered Should the text be centered? (Note - this has a performance cost, especially
   *     for the Greenfoot Gallery and doubly so for accessing the Gallery via mobile device)
   * @param width The width, in pixels, for this text box
   * @param borderThickness The thickness of the border. Setting this to zero will make a borderless
   *     text box
  ...
  public SuperTextBox(
    ...
    if (borderThickness > 0) bordered = true;
    else bordered = false;
  ...
  private GreenfootImage generateImage(
      int width, int vSpace, int padding, int borderThickness, int fontSize, int numLines) {
    height = (padding * 2) + (vSpace * (numLines)) + (fontSize * numLines) + (2 * borderThickness);
   ...
   *
   * <p>For text boxes with one line, this will insert the text at the BOTTOM.
   *
   * <p>For text boxes with just a single line, this will replace the existing text and, if
   ...
   *
   * <p>Note that if you have centering turned on this has significant CPU cost because it will call
   * the getStringWidth method as many times as there are Strings in String[] text. For example, if
   * there are four lines of text, it would call the method four times, which in the best case is
   * 12-16ms (on a fast local computer) which will almost certainly slow your acts below 60 fps. On
   * the Greenfoot Gallery, the time for this method to run is about 10x slower, or about 120-160ms,
   * which is enough for perceptible lag.
   *
   * <p>It is better to call updateLine for centered text, as it maintains the previously calculated
   * values for the existing lines and only calls the getStringWidth method once for the new text
   * inserted.
   *
   * <p>If you're not centering your text, you can ignore the above.
   */
  ...
  /**
   * Update the Multi Line Text box by adding a new line. This can add a line at the top or bottom,
   * and will move the other lines accordingly.
   *
   * @param textIn The new String to add to the display
   * @param fromBottom Should the line go at the bottom (scroll up), or false to insert it at the
   *     top (scroll down)
   */
   ...
   *
   * <p>Get the Width of a String, if it was printed out using the drawString command in a
   * particular Font.
   *
   * <p>There is a performance cost to this, although it is more significant on the Gallery, and
   * especially on the Gallery when browsed on a mobile device. It is appropriate to call this in
   * the constructor, and in most cases it is ideal NOT to call it from an act method, especially
   * every act.
   *
   * <p>In cases where values are pre-determined, it may be ideal to cache the values (save them) so
   * you don't have to run this repeatedly on the same text. If you do this in the World
   * constructor, there is no performance cost while running.
   *
   * <h3>Performance & Compatibility:</h3>
   *
   * <ul>
   *   <li>Locally, performance should be sufficient on any moderate computer (average call
   *       0.1-0.2ms on my laptop)
   *   <li>To be compatible with Greenfoot Gallery, removed use of getAwtImage() and replaced with
   *       getColorAt() on a GreenfootImage
   *   <li>On Gallery, performance is about 10x slower than locally (4ms on Gallery via Computer).
   *       For reference, an act() should be less than 16.6ms to maintain 60 frames/acts per second.
   *   <li>HUGE performance drop on Gallery via Mobile devices - not sure why, going to ignore for
   *       now. (Average update duration 34ms, more than 2 optimal acts)
   * </ul>
   *
   ...
   * @since June 2021
   * @version December 2021 - Even more Efficiency Improvement - sub 0.06ms per update on
   *     setSpeed(100)!
   */
  public static int getStringWidth(Font font, String text) {
    ...
    int marginOfError =
        fontSize / 6; // how many pixels can be skipped scanning vertically for pixels?
    int checkX;
    ...
    while (running) {
      ...
      for (int i = fontSize - 1; i >= 0 && !found; i -= marginOfError) {
        ...
        if (temp.getColorAt(checkX, i).getAlpha() != 0) {
          // This lets me only look at every other pixel on the first run - check back one to the
          // right
          // when I find a pixel to see if I passed the first pixel or not. This should almost half
          // the
